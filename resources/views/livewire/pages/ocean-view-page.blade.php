        ...
        <div class="row">
            ...
            <div class="col-12 col-lg-11 px-0 order-2 order-lg-1">
                ...
                <div class="position-relative p-0" id="inventory-container">
                    <img src="{{asset('img/fachada-vista-mar.webp')}}" alt="Inventario disponible de Espacio Marina" class="w-200 shadow-5">
    
                    <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" class="position-absolute h-100 start-0 top-0 px-0" viewBox="0 0 1920 980">
    
                        @foreach ($oceanview_units as $unit)
                            <a href="{{route('pages.unit', ['name' => $unit->name, 'contact' => request()->query('contact') ] )}}" wire:navigate class="text-decoration-none link-light" title="{{$unit->name}}">

                                <polygon class="{{ strtolower($unit->status) }}-class" points="{{$unit->shape->points ?? '0,0'}}"></polygon>
                                
                                @php
                                    if($unit->section_id == 4){
                                        $font_size = 13;
                                    }
                                    else{
                                        $font_size = 17;
                                ...
                                <text x="{{$unit->shape->text_x ?? 0;}}" y="{{$unit->shape->text_y ?? 0; }}"
                                    font-size="{{$font_size}}" font-weight="light" fill="#fff" >
            ...
            <div class="col-12 col-lg-1 px-0 order-1 order-lg-2">
                <ul class="nav nav-pills nav-inventory mb-3 justify-content-center justify-content-lg-start" id="pills-tab" role="tablist">
                    ...
                    <li class="nav-item me-4 me-lg-0" role="presentation">
                      <button class="nav-link active inventory-btns text-center"  type="button"  aria-selected="true">
                    ...
                    <li class="nav-item" role="presentation">
                      <a href="{{route('pages.golf', request()->query() )}}" class="nav-link inventory-btns text-center" wire:navigate>
